...
let timerElement = document.getElementById("timer");
    ...
    let increaseTimeButton = document.getElementById("increaseTimeButton");
    let catchObjectButton = document.getElementById("catchObjectButton");

    let timer;
    let seconds = 140; // Starting point for countdown
    let minutes = 0;
    let score = 0; // Initial score 

    // Function to update the timer every second (countdown)
    function startTimer() {
      timer = setInterval(function() {
        if (seconds <= 0) {
          clearInterval(timer); // Stop the timer when it reaches 0
          
          // Open the new page with restart button
          window.location.href = "restart.html"; // Redirect to restart page
        } else {
          seconds--;
          minutes = Math.floor(seconds / 140);
          let remainingSeconds = seconds % 140;
    ...
    // Function to increase the timer by 30 seconds (optional)
    ...
    // // Event listener for the increase button (optional)
